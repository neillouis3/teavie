...
export async function GET() {
  try {
    ...
    if (error) {
      console.error("GET /api/user/watch-later", error);
    ...
    console.error("GET /api/user/watch-later", err);
    return NextResponse.json({ error: "Fetch failed" }, { status: 500 });
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    if (Array.isArray(body.entries) && body.entries.length > 0) {
      ...
      const { error } = await supabase.from("watch_later").upsert(rows, {
        onConflict: "user_id,catalog_id",
      });

      if (error) {
        console.error("POST /api/user/watch-later bulk", error);
    ...
    const { error } = await supabase.from("watch_later").upsert(
      {
        user_id: user.id,
        catalog_id: catalogId,
    ...
    if (error) {
      console.error("POST /api/user/watch-later", error);
      return NextResponse.json({ error: "Save failed" }, { status: 500 });
    }

    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error("POST /api/user/watch-later", err);
    return NextResponse.json({ error: "Save failed" }, { status: 500 });
  }
}
...
export async function DELETE(req: Request) {
  try {
    ...
    if (error) {
      console.error("DELETE /api/user/watch-later", error);
      return NextResponse.json({ error: "Delete failed" }, { status: 500 });
    }

    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error("DELETE /api/user/watch-later", err);
    return NextResponse.json({ error: "Delete failed" }, { status: 500 });
  }
}

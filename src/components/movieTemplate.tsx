...
// Define the props type for the MovieTemplate component
interface ExploreProps {
  similarContentData: ContentItem[];
  movie: Movie | null;  // Movie details can be null if still loading
  loading: boolean;
}

const MovieTemplate: React.FC<ExploreProps> = ({ similarContentData, movie, loading }) => {
  ...
  // State for selecting the server
          ...
          <div className="w-full h-[70vh] rounded-lg flex flex-col bg-gray-500">
            {loading ? (
              ...
              <MoviePlayer
                ...
                episode={0}
                isTmdb={1}
                server={server}  // Pass the selected server to the MoviePlayer
              />
            )}
          </div>
            ...
            {loading ? (
              ...
              <div className="w-full h-18 flex flex-col mt-4">
                ...
                <div className="flex flex-row gap-2 h-fit items-center mt-2">
                  ...
                  <h1 className="text-xs">{movie?.runtime} min</h1>

      ...
      <div className={`relative z-10 ${CONTENT_INSET_X}`}>
        <div className="flex items-start gap-4 sm:gap-6 lg:gap-8">
          <aside
            ...
            aria-hidden
          >
            <div className="h-4 w-16 animate-pulse rounded bg-default-200 dark:bg-white/10" />
            {Array.from({ length: 8 }).map((_, i) => (
              <div key={i} className="h-8 w-full animate-pulse rounded-lg bg-default-200/80 dark:bg-white/5" />
            ))}
          </aside>
          <main className="min-w-0 flex-1">
            <div className="mb-5 flex items-end justify-between gap-4">
              <h1 className="sr-only">{pageName}</h1>
              <div className="h-5 w-28 animate-pulse rounded bg-default-200 dark:bg-white/10" />
              <div className="h-8 w-24 shrink-0 animate-pulse rounded-full bg-default-200 dark:bg-white/10" />
            </div>

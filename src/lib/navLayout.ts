...
/** Mobile/tablet fixed nav shell (< lg; hidden on TV). */
export const NAV_MOBILE_SHELL_CLASS =
  "nav-mobile-shell fixed inset-x-0 top-0 z-50 flex items-center gap-2 px-4 sm:px-6 lg:hidden pt-[env(safe-area-inset-top,0px)] min-h-[calc(3.5rem+env(safe-area-inset-top,0px))]";

/** Mobile nav Suspense fallback — matches shell footprint without interactive content. */
export const NAV_MOBILE_FALLBACK_CLASS =
  "nav-mobile-shell fixed inset-x-0 top-0 z-50 bg-background lg:hidden pt-[env(safe-area-inset-top,0px)] min-h-[calc(3.5rem+env(safe-area-inset-top,0px))]";

/** Desktop floating nav outer shell (lg+ or TV). */
export const NAV_DESKTOP_SHELL_CLASS =
  "nav-desktop-shell pointer-events-none fixed inset-x-0 top-0 z-50 hidden px-4 pt-3 sm:px-6 lg:flex lg:justify-center";

/** Desktop floating nav inner pill. */
export const NAV_DESKTOP_INNER_CLASS =
  "pointer-events-auto flex h-14 w-full max-w-[80rem] items-center gap-2 rounded-[1.15rem] border border-default-200/70 bg-background/88 pl-3 pr-2 text-foreground shadow-[0_8px_28px_rgba(15,23,42,0.08)] backdrop-blur-xl supports-[backdrop-filter]:bg-background/82 dark:border-white/10 dark:bg-black/45 dark:shadow-[0_14px_40px_rgba(0,0,0,0.2)] dark:supports-[backdrop-filter]:bg-background/44";
